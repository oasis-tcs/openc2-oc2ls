...
Sign up 
Branch: TobyConsidine-… 
openc2-oc2ls/oc2ls.md 
Find file 
Copy path 
 TobyConsidine Update oc2ls.md 
8817b6a 8 days ago 
6 contributors 
...
Producers: A Producer is an entity that creates Commands to provide instruction to one or more systems to act in accordance with the content of the Command. A Producer may receive and process Responses in conjunction with a Command.
Consumers: A Consumer is an entity that receives and may act upon a Command. A Consumer may create Responses that provide any information captured or necessary to send back to the Producer.
...
The Integrated Adaptive Cyber Defense (IACD) framework defines a collection of activities, based on the traditional OODA (Observeâ€“Orientâ€“Decideâ€“Act) Loop [IACD]:
...
the set of Actions and options that may be used in Commands
the set of Targets and Target Specifiers
a syntax that defines the structure of Commands and Responses
a JSON serialization of Commands and Responses
...
Language elements applicable to some Actuators, which may be defined in individual Actuator profiles.
...
The Action and Target components are required and are populated by one of the Actions in Section 3.3.1.1 and the Targets in Section 3.3.1.2. A particular Target may be further refined by the Target type definitions in Section 3.4.1. Procedures to extend the Targets are described in Section 3.1.4.
...
Any
Anything, used to designate fields with an unspecified value.
Binary
A sequence of octets. Length is the number of octets.
Boolean
An element with one of two values: true and false.
Integer
A whole number.
Number
A real number.
Null
Nothing, used to designate fields with no value.
String
...
Choice.ID
One field selected from a set of fields. The API value has an id and a type.
Enumerated
...
Map.ID
An unordered set of fields. The API value of each field has an id, label, and type.
MapOf(ktype, vtype)
...
Field definitions and API values are identified only by ID. The non-normative description may include a suggested name.
Serialized values of Enumerated types and keys of Choice/Map types are IDs regardless of serialization format.
...
Enumerated
ID, Name, Description
Array, Choice.ID, Map.ID
ID, Type, Multiplicity (#), Description
Choice, Map, Record
ID, Name, Type, Multiplicity (#), Description
...
email
String
Value must be an email address as defined in [RFC5322], Section 3.4.1
eui
Binary
Value must be an EUI-48 or EUI-64 as defined in [EUI]
hostname
String
Value must be a hostname as defined in [RFC1034], Section 3.1
idn-email
String
Value must be an internationalized email address as defined in [RFC6531]
idn-hostname
String
Value must be an internationalized hostname as defined in [RFC5890], Section 2.3.2.3
iri
String
Value must be an Internationalized Resource Identifier (IRI) as defined in [RFC3987]
uri
String
Value must be a Uniform Resource Identifier (URI) as defined in [RFC3986]
3.1.3 Multiplicity
...
Targets, defined in Section 3.3.1.2, MAY be extended. Extended Target names MUST be prefixed with a namespace identifier followed by a colon (":").
...
Command Arguments, defined in Section 3.3.1.4, MAY be extended using the namespace identifier as the Argument name, called an extended Argument namespace. Extended Arguments MUST be defined within the extended Argument namespace.
Example: In this example Command, the extended Argument, direction, is defined within the Stateless Packet Filtering Profile namespace, slpf.
{
    "action": "deny",
    "target": {
        "ipv6_net": {...}
...
Binary
JSON string containing Base64url encoding of the binary value as defined in [RFC4648], Section 5.
Binary /x
JSON string containing Base16 (hex) encoding of a binary value as defined in [RFC4648], Section 8. Note that the Base16 alphabet does not include lower-case letters.
IPv4-Addr
JSON string containing the "dotted-quad" representation of an IPv4 address as specified in [RFC2673], Section 3.2.
IPv6-Addr
JSON string containing the text representation of an IPv6 address as specified in [RFC5952], Section 4.
MAC-Addr
JSON string containing the text representation of a MAC Address in colon hexadecimal format as defined in [EUI].
Boolean
JSON true or false
Integer
JSON number
...
JSON array
Array /ipv4-net
JSON string containing the text representation of an IPv4 address range as specified in [RFC4632], Section 3.1.
Array /ipv6-net
JSON string containing the text representation of an IPv6 address range as specified in [RFC4291], Section 2.3.
ArrayOf
JSON array
Choice
JSON object with one member. Member key is the field name.
Choice.ID
JSON object with one member. Member key is the integer field id converted to string.
Enumerated
JSON string
Enumerated.ID
JSON integer
Map
JSON object. Member keys are field names.
Map.ID
JSON object. Member keys are integer field ids converted to strings.
MapOf
JSON object. Member keys are as defined in the specified key type.
Record
JSON object. Member keys are field names.
...
Type definitions and application values use only the ID. There is no corresponding name except as an optional part of the description.
Instances of Enumerated values and Choice/Map keys are serialized as IDs regardless of serialization format.
3.2 Message
...
IPv6-Connection
1
A 5-tuple of source and destination IPv6 address ranges, source and destination ports, and protocol.
20
iri
IRI
1
...
slpf:Actuator
1
Example: Actuator Specifiers defined in the Stateless Packet Filtering Profile
3.3.1.4 Command Arguments
Type: Args (Map{1..*})
ID
Name
...
IPv4-Addr
1
IPv4 address as defined in [RFC0791]
2
Integer
0..1
CIDR prefix-length. If omitted, refers to a single host address.
3.4.1.10 IPv4 Connection
...
L4-Protocol
0..1
Layer 4 protocol (e.g., TCP) - see Section 3.4.2.10
Usage Requirement:
...
IPv6-Addr
1
IPv6 address as defined in [RFC8200]
2
Integer
0..1
prefix-length. If omitted, refers to a single host address.
3.4.1.12 IPv6 Connection
...
IPv6-Net
0..1
IPv6 destination address range
4
dst_port
Port
0..1
...
L4-Protocol
0..1
Layer 4 protocol (e.g., TCP) - Section 3.4.2.10
Usage Requirement:
...
1
command
The Message content is an OpenC2 Command
2
response
The Message content is an OpenC2 Response
3.4.2.12 Namespace Identifier
Type Name
Type Definition
Description
...
The 'query features' Command is REQUIRED for all Producers. The 'query features' Command MAY include one or more Features as defined in Section 3.4.2.4. The 'query features' Command MAY include the "response_requested": "complete" Argument. The 'query features' Command MUST NOT include any other Argument.
...
5.1-4 MUST include zero or one actuator property defined in accordance with Section 3.3.1.3 or zero or one imported actuator property defined in accordance with Section 3.1.4.
...
5.3-3 MUST implement JSON serialization of received Responses in accordance with [RFC7493].
5.4 Conformance Clause 4: Consumer
...
5.4-3 MUST implement JSON serialization of received Commands in accordance with [RFC7493].
...
message=header+body
Review comments
Using word 'arguments' instead of 'options'
v1.0-csd04
5/31/2018
Romano, Sparrell
approved wd06
...
7/11/2018
Romano, Sparrell
Continued refinement of details
Review comments
Moved some Actions and Targets to reserved lists
v1.0-wd08
10/05/2018
Romano, Sparrell
Continued refinement of details
...
10/17/2018
Romano, Sparrell
Additional review comments to create wd09 for CSD approval and release for public review.
v1.0-wd10
03/04/2019
Romano, Sparrell
Produce interim working draft.
v1.0-wd11
03/21/2019
Romano, Sparrell
Produce interim working draft.
...
03/27/2019
Romano, Sparrell
Produce candidate working draft for next public review.
v1.0-wd13
05/14/2019
Romano, Sparrell
Incorporated comments from CSPRD02.
v1.0-wd14
05/23/2019
Romano, Sparrell
Incorporated comments from WD13 ballot.
v1.0-wd15
05/23/2019
Romano, Sparrell
Incorporated Errata edited by David Kemp.
